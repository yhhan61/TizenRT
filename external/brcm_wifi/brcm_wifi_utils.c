...
brcm_security_config_t *getSecurityConfig(char *sec_type, char *psk, WiFi_InterFace_ID_t mode)
{
	...
	brcm_security_config_t *ret = NULL;
	if (strncmp(BRCM_WIFI_SECURITY_OPEN, sec_type, sizeof(BRCM_WIFI_SECURITY_OPEN)) != 0) {
		ret = (brcm_security_config_t *)zalloc(sizeof(brcm_security_config_t));
		if (ret) {
			if (strncmp(BRCM_WIFI_SECURITY_WEP_OPEN, sec_type, sizeof(BRCM_WIFI_SECURITY_WEP_OPEN)) == 0) {
				ret->secmode = BRCM_SEC_MODE_WEP;
			} else if (strncmp(BRCM_WIFI_SECURITY_WEP_SHARED, sec_type, sizeof(BRCM_WIFI_SECURITY_WEP_SHARED)) == 0) {
				ret->secmode = BRCM_SEC_MODE_WEP_SHARED;
			} else if (strncmp(BRCM_WIFI_SECURITY_WPA_MIXED, sec_type, sizeof(BRCM_WIFI_SECURITY_WPA_MIXED)) == 0) {
				ret->secmode = BRCM_SEC_MODE_WPA_MIXED;
			} else if (strncmp(BRCM_WIFI_SECURITY_WPA_TKIP, sec_type, sizeof(BRCM_WIFI_SECURITY_WPA_TKIP)) == 0) {
				ret->secmode = BRCM_SEC_MODE_WPA_TKIP;
			} else if (strncmp(BRCM_WIFI_SECURITY_WPA_AES, sec_type, sizeof(BRCM_WIFI_SECURITY_WPA_AES)) == 0) {
				ret->secmode = BRCM_SEC_MODE_WPA_CCMP;
			} else if (strncmp(BRCM_WIFI_SECURITY_WPA2_MIXED, sec_type, sizeof(BRCM_WIFI_SECURITY_WPA2_MIXED)) == 0) {
				ret->secmode = BRCM_SEC_MODE_WPA2_MIXED;
			} else if (strncmp(BRCM_WIFI_SECURITY_WPA2_TKIP, sec_type, sizeof(BRCM_WIFI_SECURITY_WPA2_TKIP)) == 0) {
				ret->secmode = BRCM_SEC_MODE_WPA2_TKIP;
			} else if (strncmp(BRCM_WIFI_SECURITY_WPA2_AES, sec_type, sizeof(BRCM_WIFI_SECURITY_WPA2_AES)) == 0) {
				ret->secmode = BRCM_SEC_MODE_WPA2_CCMP;
			}
		}
...
static void getSecurityModeString(brcm_security_config_t *sec, uint8_t count, char *modestring)
{
	...
	if (count == 0) {
		...
		for (x = 0; x < count; x++) {
			strncat(pos, "[", 1);
			pos += 1;
			if (sec->secmode == BRCM_SEC_MODE_WEP) {
				strncat(pos, "WEP", 3);
				pos += 3;
			} else if (sec->secmode == BRCM_SEC_MODE_WEP_SHARED) {
				strncat(pos, "WEP_SHARED", 10);
				pos += 10;
			} else if (sec->secmode == (BRCM_SEC_MODE_WEP | BRCM_SEC_MODE_WEP_SHARED)) {
				strncat(pos, "WEP", 3);	// we dont really know if this is open or shared
				// let's just yse "WEP"
				pos += 3;
			} else if (sec->secmode == BRCM_SEC_MODE_WPA_TKIP) {
				strncat(pos, "WPA-PSK+TKIP", 12);
				pos += 12;
			} else if (sec->secmode == BRCM_SEC_MODE_WPA_CCMP) {
				strncat(pos, "WPA-PSK+AES", 11);
				pos += 11;
			} else if (sec->secmode == BRCM_SEC_MODE_WPA_MIXED) {
				strncat(pos, "WPA-PSK+AES+TKIP", 16);
				pos += 16;
			} else if (sec->secmode == BRCM_SEC_MODE_WPA2_TKIP) {
				strncat(pos, "WPA2-PSK+TKIP", 13);
				pos += 13;
			} else if (sec->secmode == BRCM_SEC_MODE_WPA2_CCMP) {
				strncat(pos, "WPA2-PSK+AES", 12);
				pos += 12;
			} else if (sec->secmode == BRCM_SEC_MODE_WPA2_MIXED) {
				strncat(pos, "WPA2-PSK+AES+TKIP", 17);
				pos += 17;
			} else if (sec->secmode == BRCM_SEC_MODE_EAP) {
...
void printScanResult(brcm_scan_info_t *list)
{
	...
	brcm_scan_info_t *current_element = list;
