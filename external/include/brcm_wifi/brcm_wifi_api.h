...
#ifndef BRCM_WIFI_API_H_
#define BRCM_WIFI_API_H_
...
typedef enum {
	BRCM_SEC_MODE_OPEN			= 0,				// 00000000
	BRCM_SEC_MODE_WEP			= BIT(1),			// 00000010
	BRCM_SEC_MODE_WEP_SHARED	= BIT(2),			// 00000100
	BRCM_SEC_MODE_WPA_TKIP		= BIT(3),			// 00001000
	BRCM_SEC_MODE_WPA_CCMP		= BIT(4),			// 00010000
	BRCM_SEC_MODE_WPA_MIXED		= (BIT(3) | BIT(4)),// 00011000
	BRCM_SEC_MODE_WPA2_TKIP		= BIT(5),			// 00100000
	BRCM_SEC_MODE_WPA2_CCMP		= BIT(6),			// 01000000
	BRCM_SEC_MODE_WPA2_MIXED	= (BIT(5) | BIT(6)),// 01100000
	BRCM_SEC_MODE_EAP			= BIT(7)			// 10000000
} brcm_security_mode_t;

/* Length defines */
#define BRCM_MACADDR_LEN        6
#define BRCM_MACADDR_STR_LEN    18
/* The maximum length of an SSID - excluding '\0' is case of all characters used */
#define BRCM_SSID_LEN           32
#define BRCM_PASSPHRASE_LEN     64

/* BRCM return values */
#define BRCM_STATUS_SUCCESS                             0	// Successfully completed
#define BRCM_STATUS_ERROR                               1	// Error  - unspecified
#define BRCM_STATUS_COMMAND_FAILED                      2	// Failed - command failed
#define BRCM_STATUS_COMMAND_UNKNOWN                     3	// Failed - command unknown
#define BRCM_STATUS_NOT_STARTED                         4	// Failed - mode never initiated
#define BRCM_STATUS_ALREADY_STARTED                     5	// Failed - mode already started
#define BRCM_STATUS_SUPPLICANT_START_FAILED             6	// Failed - start up of wpa_supplicant failed
#define BRCM_STATUS_PARAM_FAILED                        7	// Failed - parameter specified not valid
#define BRCM_STATUS_ALREADY_CONNECTED                   8	// Failed - WiFi already connected
#define BRCM_STATUS_NOT_CONNECTED                       9	// Failed - WiFi not connected
#define BRCM_STATUS_SECURITY_FAILED                     10	// Failed - security setup failed
#define BRCM_STATUS_NOT_ALLOWED                         11	// Failed - not allowed
#define BRCM_STATUS_NOT_SUPPORTED                       12	// Failed - function not supported (maybe due to missing dependencies to filesystem)

/* BRCM reason codes */
/* Return values used from ieee802_11_defs.h
 * (Reason codes (IEEE 802.11-2007, 7.3.1.7, Table 7-22)
 * BRCM Wi-Fi own defined reason codes are starting from 200
 */
/* Added in Join failed scenarios:*/
#define BRCM_REASON_NETWORK_CONFIGURATION_NOT_FOUND 201
#define BRCM_REASON_NETWORK_AUTHENTICATION_FAILED   202
#define BRCM_REASON_ASSOCIATION_REQ_FAILED          203
 ...
 */
#define BRCM_SCAN_INTERVAL          30
#define BRCM_SCAN_INTERVAL_CONNECT  10

/**
 * Specify expiration of scans results in lower layers
 * Value is in seconds (default is 120 seconds)
 */
#define BRCM_BSS_EXPIRE_AGE         120
 ...
 * Values: 3 is default
 */
#define BRCM_STA_JOIN_SCAN_ATTEMPT  3

/* brcm wifi network interface - api parameter defines */
typedef enum WiFi_InterFace_ID {
	BRCM_WIFI_NONE,
	BRCM_WIFI_STATION_IF,	// Station mode (turns on wpa_supplicant)
	BRCM_WIFI_SOFT_AP_IF,	// Soft AP mode (turns on hostapd)
	BRCM_WIFI_P2P_IF		// P2P mode (turns on wpa_supplicant)
} WiFi_InterFace_ID_t;
...
typedef struct brcm_security_config {
	uint32_t secmode;
	char passphrase[BRCM_PASSPHRASE_LEN];
} brcm_security_config_t;

typedef struct brcm_vendor_ie {
...
typedef struct brcm_ap_config {
	...
	brcm_ht_config_t ht_mode;			// requires CONFIG_HT_OVERRIDES enabled, see brcm_ht_config_t
	brcm_security_config_t *security;	// use NULL if security mode is open
	brcm_vendor_ie_t *vsie;				// Vender specific IE block
} brcm_ap_config_t;

typedef struct brcm_scan_info {
	uint8_t ssid[BRCM_SSID_LEN + 1];	// 802.11 spec defined unspecified or uint8
	char bssid[BRCM_MACADDR_STR_LEN];	// char string e.g. xx:xx:xx:xx:xx:xx
	int8_t ssid_len;					// length of ssid - # of valid octets
	...
	brcm_ht_config_t ht_mode;			// See brcm_ht_config_t
	brcm_security_config_t *sec_modes;	// list of security modes
	struct brcm_scan_info *next;
} brcm_scan_info_t;

typedef struct brcm_reason {
	uint32_t reason_code;				// Reason codes - 0 for success - error code see 'BRCM reason codes' above
	uint8_t locally_generated;			// Which side cause link down, 1 = locally, 0 = remotely - valid for STA mode only
	int8_t ssid_len;					// length of ssid - # of valid octets
	uint8_t ssid[BRCM_SSID_LEN + 1];	// 802.11 spec defined up to 32 octets of data
	char bssid[BRCM_MACADDR_STR_LEN];	// BSS identification, char string e.g. xx:xx:xx:xx:xx:xx
} brcm_reason_t;

typedef int8_t (*network_scan_result_handler_t)(brcm_reason_t *reason);
typedef void (*brcm_network_link_callback_t)(brcm_reason_t *reason);
 ...
 * status occurs, these carry a brcm_reason_t to tell why link is up or down
 */
 ...
 * results are returned as the list of structure brcm_ap_info_t linked via next
 * pointer.
 ...
 * @security_config    a pointer to data to brcm_security_config_t containing passphrase, keymgmt and cipher.
 ...
 */
int8_t WiFiIsConnected(uint8_t *count, brcm_reason_t *details);

/**
 * Request the current mode of operation.
 *  @mode:       sets to one of BRCM_WIFI_NONE, BRCM_WIFI_STATION_IF,
 *               BRCM_WIFI_SOFT_AP_IF
 * Return: success or failure
 */
