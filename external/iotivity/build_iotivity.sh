...
IOTIVITY_TARGET_BOARD=$(echo "$CONFIG_ARCH_BOARD" | sed 's/"//g')
if [ ${IOTIVITY_TARGET_BOARD} != "rp0w" ]; then
	IOTIVITY_TARGET_BOARD="sidk_s5jt200"
fi
...
if [ ${CONFIG_ENABLE_IOTIVITY} -eq 1 ]; then
	...
	OPTIONS="TARGET_OS=tizenrt TARGET_ARCH=${IOTIVITY_TARGET_ARCH} TARGET_TRANSPORT=IP ROUTING=${IOTIVITY_ROUTING} TC_PREFIX=arm-none-eabi- VERBOSE=no TIZENRT_OS_DIR=${TOPDIR} PLATFORM_TLS=${PLATFORM_TLS} ARCH_BOARD=${IOTIVITY_TARGET_BOARD} -j 8"
