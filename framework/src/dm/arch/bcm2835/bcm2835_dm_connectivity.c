...
#include <brcm_wifi/brcm_wifi_utils.h>
#include <arpa/inet.h>
 ...
 */
static brcm_scan_info_t *g_wifi_scan_result;
static brcm_scan_info_t *g_wifi_scan_iter;
...
static void dm_cb_linkUpEvent(brcm_reason_t *reason);
static void dm_cb_linkDownEvent(brcm_reason_t *reason);
...
void dm_cb_register_init()
{
	...
	if (WiFiRegisterLinkCallback(&dm_cb_linkUpEvent, &dm_cb_linkDownEvent) != BRCM_STATUS_SUCCESS)
...
int dm_conn_get_rssi(int *rssi)
{
	...
	if ((ret = wwd_wifi_get_rssi(&val)) != WWD_SUCCESS) {
		dmdbg("dm_log : ret[%d]\n", ret);
...
int dm_conn_get_channel(int *channel)
{
	...
	if ((ret = wwd_wifi_get_channel(WWD_STA_INTERFACE, &val)) != WWD_SUCCESS) {
		dmdbg("dm_log : ret[%d]\n", ret);
		return DM_ERROR_NO_DATA;
	}
	*channel = (int)val;
...
int dm_conn_get_tx_power(int *dbm)
{
	...
	if ((ret = wwd_wifi_get_tx_power(&val)) != WWD_SUCCESS) {
		dmdbg("dm_log : ret[%d]\n", ret);
		return DM_ERROR_NO_DATA;
	}
	*dbm = (int)val;
...
int dm_conn_set_tx_power(const int *dbm)
{
	...
	val = (uint8_t)(*dbm);
	if ((ret = WiFiSetTxPower(&val)) != BRCM_STATUS_SUCCESS) {
		dmdbg("dm_log : ret[%d]\n", ret);
		return DM_ERROR_INVALID_PARAMETER;
	}
	return DM_ERROR_NONE;
...
static int8_t prv_dm_conn_scan_result(brcm_reason_t *reason)
{
	if (reason->reason_code != 0) {
		...
		WiFiNetworkJoin((uint8_t *)CONFIG_DM_AP_SSID, strlen(CONFIG_DM_AP_SSID), \
						NULL, (brcm_security_config_t *)getSecurityConfig(CONFIG_DM_AP_SECURITY, \
								CONFIG_DM_AP_PASS, BRCM_WIFI_STATION_IF));
	}
...
int dm_conn_get_scan_result(dm_scan_info_t **result)
{
	...
	do {
		...
		memset(curr_record->ssid, 0x00, BRCM_SSID_LEN + 1);
...
{
	if (WiFiScanNetwork() == BRCM_STATUS_SUCCESS) {
		WiFiFreeScanResults(&g_wifi_scan_result);
		WiFiGetScanResults(&g_wifi_scan_result);
		WiFiNetworkJoin((uint8_t *)CONFIG_DM_AP_SSID, strlen(CONFIG_DM_AP_SSID), \
						NULL, (brcm_security_config_t *)getSecurityConfig(CONFIG_DM_AP_SECURITY, \
...
int dm_conn_wifi_connect(conn_cb linkUpEvent, conn_cb linkDownEvent)
{
	...
	if (ret == BRCM_STATUS_SUCCESS) {
		dmdbg("Wi-Fi status - Already Connected : %d\n", result);
		g_dm_connected = 1;
		return 1;
	} else if (ret != BRCM_STATUS_NOT_STARTED)
		return DM_ERROR_UNKNOWN;


	ret = WiFiStart(BRCM_WIFI_STATION_IF, NULL);
	if (ret == BRCM_STATUS_SUCCESS) {
		dmdbg("STA mode started\n");
		ret = WiFiNetworkJoin((uint8_t *)CONFIG_DM_AP_SSID, strlen(CONFIG_DM_AP_SSID), \
							  NULL, (brcm_security_config_t *)getSecurityConfig(CONFIG_DM_AP_SECURITY, CONFIG_DM_AP_PASS, BRCM_WIFI_STATION_IF));
		sleep(1);
		if (ret == BRCM_STATUS_SUCCESS) {
