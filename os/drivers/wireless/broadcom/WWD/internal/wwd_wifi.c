                          ...
                          )
{
    ...
    {
        memcpy( scan_params->params.bssid.ether_addr_octet, optional_mac, sizeof(wiced_mac_t) );
    }
    else
    {
        memset( scan_params->params.bssid.ether_addr_octet, 0xff, sizeof(wiced_mac_t) );
    }
...
static void* wwd_scan_events_handler( const wwd_event_header_t* event_header, const uint8_t* event_data, /*@returned@*/ void* handler_user_data )
{
    ...
    if ( event_header->event_type == WLC_E_PFN_NET_FOUND )
    {
        ...
        memcpy( record->BSSID.octet, pfn_subnet->BSSID.ether_addr_octet, sizeof(record->BSSID.octet) );
...
//    memcpy(&record->bi, bss_info, sizeof(wl_bss_info_t));
    record->bi = bss_info;
#endif

    version = WICED_READ_32( &bss_info->version );
#ifdef ARASAN_EMMC_SDIO_CYW43438
#define _ARASAN_BSS_INFO_VERSION_MASK 0x10000
    version &= ~_ARASAN_BSS_INFO_VERSION_MASK;
#endif
    ...
    memcpy( (void*) record->BSSID.octet, (const void*) bss_info->BSSID.ether_addr_octet, sizeof(bss_info->BSSID.ether_addr_octet) );
...
/*
    wiced_buffer_t        buffer1;
    wlc_ssid_t*           ssid_params1;
*/
...
static wwd_result_t wwd_wifi_prepare_join( wwd_interface_t interface, wiced_security_t auth_type, /*@unique@*/ const uint8_t* security_key, uint8_t key_length, host_semaphore_type_t* semaphore )
{
    ...
    *wsec_setting = (uint32_t) ((auth_type&0xFF) & ~WPS_ENABLED);
    CHECK_RETURN( wwd_sdpcm_send_ioctl( SDPCM_SET, WLC_SET_WSEC, buffer, 0, interface ) );      // OK
    ...
    (void)wwd_sdpcm_send_iovar( SDPCM_SET, buffer, 0, WWD_STA_INTERFACE );      // NOK (IOCTL failed)
    ...
    switch ( auth_type )
    {
        ...
        case WICED_SECURITY_WPA2_MIXED_PSK:
            ...
            if (wwd_wifi_set_passphrase( security_key, key_length, interface ) != WWD_SUCCESS) {
...
//            CHECK_RETURN( wwd_wifi_set_passphrase( security_key, key_length, interface ) );
    ...
    *infra = (uint32_t) ( ( auth_type & IBSS_ENABLED ) == 0 )? 1 : 0;
    ...
    wpa_auth = (uint32_t*) wwd_sdpcm_get_ioctl_buffer( &buffer, (uint16_t) 4 );     // NOK (Timeout)
    ...
    wwd_sdpcm_send_ioctl( SDPCM_SET, WLC_SET_WPA_AUTH, buffer, 0, interface );
...
static /*@null@*/ void* wiced_join_events_handler( const wwd_event_header_t* event_header, const uint8_t* event_data, /*@returned@*/ void* handler_user_data )
{
    ...
    switch ( event_header->event_type )
    {
        ...
        case WLC_E_LAST:
        default:
            WPRINT_WWD_DEBUG(("Received event which was not registered\n"));
